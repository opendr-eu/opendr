...
**Returns**:
  ...
  - **performance**: *dict*  
    A dictionary that holds the performance curves with the following keys:  
        - `backbone_performance`: a *dict* that contains `"train_cross_entropy"`,  `"val_cross_entropy"`, ...  when training the backbone classifier  
        - `initialization_performance`: a *dict* that contains `"train_mean_squared_error"` .. when training the teacher's sensing and synthesis components  
        - `compressive_learning_performance`: a *dict* that contains `"train_cross_entropy"`, `"train_acc"`, ... when training the compressive model  
...
This method is used to download CIFAR10 and CIFAR100 pretrained models for the `cifar_allcnn` architecture. Pretrained models are available for `backbone='cifar_allcnn` and `n_class in [10, 10]` and `compressed_shape in [(20, 19, 2), (28, 27, 1), (14, 11, 2), (18, 17, 1), (9, 6, 1), (6, 9, 1)]`. Here we should note that the input image to the pretrained model should be scaled to the range [0, 1] and standardized using  `mean = [0.491372, 0.482352, 0.446666]` and `std = [0.247058, 0.243529, 0.261568]`  
