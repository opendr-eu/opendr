        ...
        for i in range(len(verts_smooth)):
            ...
            for j in range(len(nrml_ids[i])):
                ...
                if dnorm_new > 0 and np.dot(nrmls[i], d_vt_normalized) > 0 and linalg.norm(disps_final[i]) > 0:
                    an = [0.99, 0.01]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1]),
                    1:4] = an[0] * deul_new + an[1] * disp_eul[
                        int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1])]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]) - 1,
                    int(vt[0] * disp_norms.shape[1]) - 1, 1:4] = an[0] * deul_new + an[1] * disp_eul[
                        int((1 - vt[1]) * disp_norms.shape[0]) - 1, int(vt[0] * disp_norms.shape[1]) - 1]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]) - 1,
                    int(vt[0] * disp_norms.shape[1]), 1:4] = an[0] * deul_new + an[1] * disp_eul[
                        int((1 - vt[1]) * disp_norms.shape[0]) - 1, int(vt[0] * disp_norms.shape[1])]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]) - 1,
                    min(disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1), 1:4] = an[0] * deul_new + an[
                        1] * disp_eul[int((1 - vt[1]) * disp_norms.shape[0]) - 1, min(disp_norms.shape[1] - 1, int(
                        vt[0] * disp_norms.shape[1]) + 1)]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]),
                    int(vt[0] * disp_norms.shape[1]) - 1, 1:4] = an[0] * deul_new + an[1] * disp_eul[
                        int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1]) - 1]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]),
                    min(disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1), 1:4] = an[0] * deul_new + an[
                        1] * disp_eul[int((1 - vt[1]) * disp_norms.shape[0]), min(disp_norms.shape[1] - 1,
                                                                                  int(vt[0] * disp_norms.shape[1]) + 1)]
                    displacement_map_smoothed[min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1),
                    int(vt[0] * disp_norms.shape[1]) - 1, 1:4] = an[0] * deul_new + an[1] * disp_eul[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), int(
                            vt[0] * disp_norms.shape[1]) - 1]
                    displacement_map_smoothed[min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1),
                    int(vt[0] * disp_norms.shape[1]), 1:4] = an[0] * deul_new + an[1] * disp_eul[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), int(
                            vt[0] * disp_norms.shape[1])]
                    displacement_map_smoothed[min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1),
                    min(disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1), 1:4] = an[0] * deul_new + an[
                        1] * disp_eul[min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), min(
                        disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1)]
                    displacement_map_smoothed[
                        int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1]), 0] = an[
                                                                                                           0] * dnorm_new + \
                                                                                                       an[1] * \
                                                                                                       disp_norms[int(
                                                                                                           (1 - vt[1]) *
                                                                                                           ...
                                                                                                           disp_norms.shape[
                                                                                                               1])]
                    displacement_map_smoothed[
                        int((1 - vt[1]) * disp_norms.shape[0]) - 1, int(vt[0] * disp_norms.shape[1]) - 1, 0] = an[
                                                                                                                   0] * dnorm_new + \
                                                                                                               an[1] * \
                                                                                                               disp_norms[
                                                                                                                   int((
                                                                                                                       ...
                                                                                                                       disp_norms.shape[
                                                                                                                           1]) - 1]
                    displacement_map_smoothed[
                        int((1 - vt[1]) * disp_norms.shape[0]) - 1, int(vt[0] * disp_norms.shape[1]), 0] = an[
                                                                                                               0] * dnorm_new + \
                                                                                                           an[1] * \
                                                                                                           disp_norms[
                                                                                                               int((1 -
                                                                                                                   ...
                                                                                                                   disp_norms.shape[
                                                                                                                       1])]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]) - 1, min(disp_norms.shape[1] - 1,
                                                                                              int(vt[0] *
                                                                                                  disp_norms.shape[
                                                                                                      1]) + 1), 0] = an[
                                                                                                                         0] * dnorm_new + \
                                                                                                                         ...
                                                                                                                         int(
                                                                                                                             ...
                                                                                                                             int(
                                                                                                                                 ...
                                                                                                                                 disp_norms.shape[
                                                                                                                                     1]) + 1)]
                    displacement_map_smoothed[
                        int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1]) - 1, 0] = an[
                                                                                                               0] * dnorm_new + \
                                                                                                           an[1] * \
                                                                                                           disp_norms[
                                                                                                               int((1 -
                                                                                                                   ...
                                                                                                                   disp_norms.shape[
                                                                                                                       1]) - 1]
                    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]), min(disp_norms.shape[1] - 1, int(
                        vt[0] * disp_norms.shape[1]) + 1), 0] = an[0] * dnorm_new + an[1] * disp_norms[
                        int((1 - vt[1]) * disp_norms.shape[0]), min(disp_norms.shape[1] - 1,
                                                                    int(vt[0] * disp_norms.shape[1]) + 1)]
                    displacement_map_smoothed[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), int(
                            vt[0] * disp_norms.shape[1]) - 1, 0] = an[0] * dnorm_new + an[1] * disp_norms[
                    ...
                    displacement_map_smoothed[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), int(
                            vt[0] * disp_norms.shape[1]), 0] = an[0] * dnorm_new + an[1] * disp_norms[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), int(
                            vt[0] * disp_norms.shape[1])]
                    displacement_map_smoothed[
                        min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), min(
                            disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1), 0] = an[0] * dnorm_new + an[
                        1] * disp_norms[min(disp_norms.shape[0] - 1, int((1 - vt[1]) * disp_norms.shape[0]) + 1), min(
                        disp_norms.shape[1] - 1, int(vt[0] * disp_norms.shape[1]) + 1)]
                # else:
                #    displacement_map_smoothed[int((1 - vt[1]) * disp_norms.shape[0]), int(vt[0] * disp_norms.shape[1])]= self.model_params["displacements"][int((1 - vt[1]) * disp_norms.shape[0])][int(vt[0]*disp_norms.shape[1])]
