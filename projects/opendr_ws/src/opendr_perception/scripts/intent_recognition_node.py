...
class IntentRecognitionNode:
    ...
    def __init__(self, input_transcription_topic="/opendr/transcription",
        ...
        :type cache_path: str
        """
        self.input_transcription_topic = input_transcription_topic

        if output_intent_topic is not None:
            self.intent_publisher = rospy.Publisher(output_intent_topic, ObjectHypothesis, queue_size=1)
        else:
    ...
    def callback(self, data):
        ...
        if self.intent_publisher is not None:
            for prediction in predictions:
                self.intent_publisher.publish(self.bridge.to_ros_category(prediction))
...
def main():
    ...
    intent_node = IntentRecognitionNode(device=device, text_backbone=args.text_backbone,
