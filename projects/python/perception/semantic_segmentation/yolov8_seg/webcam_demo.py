...
if __name__ == "__main__":
    ...
    try:
        ...
        for img in image_provider:
            ...
            # visualization_img = np.uint8(0.4 * img.opencv() + 0.6 * segmentation_img)
            ...
            if counter < 5:
                ...
                final_img = cv2.putText(visualization_img, "FPS: %.2f" % (avg_fps,), (50, 50), cv2.FONT_HERSHEY_SIMPLEX,
                                        1, (255, 0, 0), 2, cv2.LINE_AA)
            # Display the annotated frame
            if final_img is not None:
                cv2.imshow('Heatmap', final_img)
                cv2.waitKey(1)
