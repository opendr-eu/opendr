...
parser.add_argument("--tmp_path", help="Temporary path where weights will be saved", type=str,
                    default=os.path.join(OPENDR_HOME,
                                         'projects/python/perception/object_detection_2d/nms/fseq2_nms/tmp'))
parser.add_argument("--checkpoint_freq", help="Frequency in-between checkpoint saving", type=int,
                    default=1)
parser.add_argument("--resume-from", help="Epoch to load checkpoint file and resume training from", type=int,
                    default=0)
...
parser.add_argument("--max_dt_boxes", help="Maximum number of input RoIs fed to Seq2Seq-NMS", type=int,
                    default=500)
