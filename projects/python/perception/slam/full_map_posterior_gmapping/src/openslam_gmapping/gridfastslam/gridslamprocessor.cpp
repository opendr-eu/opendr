...
namespace GMapping {
  ...
  void GridSlamProcessor::setMatchingParameters(double urange, double range, double sigma, int kernsize, double lopt,
    ...
    m_matcher.setMatchingParameters(urange, range, sigma, kernsize, lopt, aopt, iterations, likelihoodSigma, likelihoodSkip,
                                    mapModel, particleWeighting, overconfidenceUniformWeight);
    if (m_infoStream)
  ...
  void GridSlamProcessor::init(unsigned int size, double xmin, double ymin, double xmax, double ymax, double delta,
                               OrientedPoint initialPose, ScanMatcherMap::MapModel mapModel, double alpha0, double beta0,
    ...
    m_delta = delta;
    if (m_infoStream)
      m_infoStream << " -xmin " << m_xmin << " -xmax " << m_xmax << " -ymin " << m_ymin << " -ymax " << m_ymax << " -delta "
                   << m_delta << " -particles " << size << endl;

    m_particles.clear();
    TNode *node = new TNode(initialPose, 0, 0, 0);
    ScanMatcherMap lmap(Point(xmin + xmax, ymin + ymax) * .5, xmax - xmin, ymax - ymin, delta, mapModel);
  ...
  const OrientedPoint GridSlamProcessor::getBestPose() const { return m_particles[getBestParticleIndex()].pose; }

  const ScanMatcherMap GridSlamProcessor::getBestMap() const { return m_particles[getBestParticleIndex()].map; }
