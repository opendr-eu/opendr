  ...
  istream &CarmenConfiguration::load(istream &is) {
    ...
    while (is) {
      ...
      if (qualifier == "ROBOTLASER1") {
        ...
        lis >> laser_type >> start_angle >> field_of_view >> angular_resolution >> maximum_range >> accuracy >>
    ...
    if (key != end() && key->second.front() == "on") {
      ...
      if (key != end()) {
        ...
        if ((soff.size() / 3 < sonar_num)) {
          cerr << __PRETTY_FUNCTION__ << ": Error " << soff.size() << " parameters for defining the sonar offsets"
               << " while the specified number of sonars requires " << sonar_num * 3 << " parameters at least" << endl;
        } else {
          cerr << __PRETTY_FUNCTION__ << ": Ok " << soff.size() << " parameters for defining the sonar offsets of " << sonar_num
               << " devices" << endl;
        }
