...
namespace GMapping {
  ...
  AutoVal::AutoVal(const std::string &value) { m_value = value; }
  ...
  AutoVal::operator std::string() const { return m_value; }

  AutoVal::operator double() const { return atof(m_value.c_str()); }

  AutoVal::operator int() const { return atoi(m_value.c_str()); }

  AutoVal::operator unsigned int() const { return (unsigned int)atoi(m_value.c_str()); }
  ...
  ConfigFile::ConfigFile(const std::string &configFile) { read(configFile); }

  ConfigFile::ConfigFile(const char *configFile) { read(configFile); }

  bool ConfigFile::read(const char *configFile) { return read(std::string(configFile)); }
  ...
  const AutoVal &ConfigFile::value(const std::string &section, const std::string &entry) const {
    std::map<std::string, AutoVal>::const_iterator ci = m_content.find(toLower(section) + '/' + toLower(entry));
    if (ci == m_content.end())
  ...
  void ConfigFile::dumpValues(std::ostream &out) {
    for (std::map<std::string, AutoVal>::const_iterator it = m_content.begin(); it != m_content.end(); it++) {
