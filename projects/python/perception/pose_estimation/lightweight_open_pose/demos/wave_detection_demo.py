...
def wave_detector(frame_list):
    pose_waves = {}  # pose_id: waving (waving = 1, not waving = 0, can't detect = -1)
    # Loop through pose ids in last frame to check for waving in each one
    for pose_id in frame_list[-1].keys():
        pose_waves[pose_id] = 0
        ...
        for frame in frame_list:
            try:
                ...
                pose_waves[pose_id] = -1
        r_wri_avg_pos = [r_wri_avg_pos[0] / len(frame_list), r_wri_avg_pos[1] / len(frame_list)]
        l_wri_avg_pos = [l_wri_avg_pos[0] / len(frame_list), l_wri_avg_pos[1] / len(frame_list)]
        ...
        if len(frame_list) > 0:
            pose = frame_list[-1][pose_id]
        else:
            pose_waves[pose_id] = -1
        r_wri_height, l_wri_height = r_wri_avg_pos[1], l_wri_avg_pos[1]
        nose_height, neck_height = pose["nose"][1], pose["neck"][1]

        if nose_height == -1 or neck_height == -1:
            # Can't detect upper pose (neck-nose), can't assume waving
            pose_waves[pose_id] = -1
        if r_wri_height == 0 and l_wri_height == 0:
            # Can't detect wrists, can't assume waving
            pose_waves[pose_id] = -1

        # Left or right wrist has height between nose's height or neck's height
        if nose_height < l_wri_height < neck_height:
            l_wrist_height_detected = True
        if nose_height < r_wri_height < neck_height:
            r_wrist_height_detected = True
        ...
        # Check for wrist movement over multiple frames, in the first pose detected
        ...
        r_wri_x_pos_std, l_wri_x_pos_std = 0, 0
        if len(r_wri_x_positions) > len(frame_list) / 2:
            r_wri_x_pos_std = std(r_wri_x_positions)
            if r_wri_x_pos_std > std_threshold:
                if r_wrist_height_detected:
                    r_wrist_movement_detected = True
        if len(l_wri_x_positions) > len(frame_list) / 2:
            l_wri_x_pos_std = std(l_wri_x_positions)
            if l_wri_x_pos_std > std_threshold:
                if l_wrist_height_detected:
                    l_wrist_movement_detected = True

        if r_wrist_height_detected and r_wrist_movement_detected:
            pose_waves[pose_id] = 1
        elif l_wrist_height_detected and l_wrist_movement_detected:
            pose_waves[pose_id] = 1
    return pose_waves
...
try:
    ...
    for img in image_provider:
        ...
        for pose_id, pose in poses.items():
            ...
            cv2.putText(img, str(pose.id), (x - 5, y - 5), cv2.FONT_HERSHEY_SIMPLEX,
