...
def curve_fitting(points, samples, index):
  ...
  for i in range(functions.shape[0]):
    ...
    for j in range(samples_per_segment):
      t = step*j
      rawcurve[i*samples_per_segment+j,:] = np.asarray([functions[i, 1]+functions[i, 2]*t+functions[i, 3]*t*t+functions[i, 4]*t*t*t, functions[i, 5] + functions[i, 6]*t + functions[i, 7]*t*t+functions[i, 8]*t*t*t])
