...
class LinearPlanner : public BaseGripperPlanner {
  private:
    ...
    static tf::Quaternion getRot(const tf::Transform &initial, const tf::Transform &next, const tf::Transform &goal, double initial_dist);

    GripperPlan calcNextStep(const GripperPlan &prev_plan, const double &dt, const double &min_velocity, const double &max_velocity);

  public:
    LinearPlanner(const std::vector<double> &gripper_goal_wrist,
