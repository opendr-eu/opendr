...
class RobotEnv : ROSCommonNode {
  ...
  protected:
    ...
    virtual geometry_msgs::Twist calcDesiredBaseTransform(std::vector<double> &base_actions,
                                                          tf::Vector3 planned_gripper_vel_rel,
                                                          tf::Transform &desired_base_tf,
                                                          double transition_noise_base,
                                                          double &regularization,
                                                          double const &last_dt) = 0;
  ...
  public:
    ...
    RobotObs reset(const std::vector<double> &initial_base_pose,
...
class RobotOmniDrive : public RobotEnv {
  protected:
    geometry_msgs::Twist calcDesiredBaseTransform(std::vector<double> &base_actions,
                                                  tf::Vector3 planned_gripper_vel_rel,
                                                  tf::Transform &desired_base_tf,
                                                  double transition_noise_base,
                                                  double &regularization,
                                                  double const &last_dt) override;

  public:
    RobotOmniDrive(uint32_t seed,
                   std::string strategy,
                   std::string world_type,
                   bool init_controllers,
                   double penalty_scaling,
