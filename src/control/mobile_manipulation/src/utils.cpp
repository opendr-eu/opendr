...
namespace utils {
    ...
    double clampDouble(double value, double min_value, double max_value) { return std::max(std::min(value, max_value), min_value); }

    tf::Transform tipToGripperGoal(const tf::Transform &gripperTipGoalWorld,
    ...
    tf::Transform gripperToTipGoal(const tf::Transform &gripperWristGoalWorld,
                                   const tf::Vector3 &tip_to_gripper_offset,
                                   const tf::Quaternion &gripper_to_base_rot_offset) {
