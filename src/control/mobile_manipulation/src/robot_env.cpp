...
bool RobotEnv::setInitialGripperPose(const std::string &initial_joint_distribution) {
    if (initial_joint_distribution == "fixed") {
        ...
        while (invalid) {
            ...
            state_copy.setVariablePosition("world_joint/theta", robot_state_.base_tf.getRotation().getAngle() * robot_state_.base_tf.getRotation().getAxis().getZ());
...
RobotObs RobotEnv::reset(const std::vector<double> &initial_base_pose,
                         const std::string &initial_joint_distribution,
...
RobotObs RobotEnv::step(std::vector<double> base_actions, const std::vector<double> &ee_velocities_world, double transition_noise_base) {
    ...
    geometry_msgs::Twist base_cmd_rel =
        calcDesiredBaseTransform(base_actions,
                                 utils::normScaleVel(planned_gripper_vel_rel, 0.0, robo_config_.base_vel_rng),
                                 desired_base_tf,
    ...
    // if (!found_ik) {
    //     // planning_scene_monitor_->getPlanningScene()->getCurrentState().copyJointGroupPositions(joint_model_group_, robot_state_.joint_values);
    // }
...
bool RobotEnv::findIk(const Eigen::Isometry3d &desired_state_eigen, const tf::Transform &desiredGripperTfWorld) {
    ...
    if (perform_collision_check_) {
        bool success = kinematic_state_->setFromIK(joint_model_group_, desired_state_eigen, robo_config_.kinematics_solver_timeout, constraint_callback_fn_);
        if (!success) {
...
void RobotEnv::addTrajectoryPoint(const tf::Transform &desired_gripper_tf, bool found_ik) {
    ...
    if (((pathPoints_.size() % (int)nthpoint) == 0) || !found_ik) {
        ...
        visualization_msgs::Marker marker =
            utils::markerFromTransform(desired_gripper_tf, "gripper_plan", utils::getColorMsg(ik_color, 0.5), mid, robo_config_.frame_id);
        gripper_visualizer_.publish(marker);
        ...
        visualization_msgs::Marker base_marker =
            utils::markerFromTransform(robot_state_.base_tf, "base_actual", utils::getColorMsg("yellow", 0.5), mid, robo_config_.frame_id);
...
void RobotEnv::publishMarker(const std::vector<double> &marker_tf,
                             int marker_id,
    ...
    visualization_msgs::Marker marker = utils::markerFromTransform(t, name_space, c, marker_id, robo_config_.frame_id, geometry, scale);
    gripper_visualizer_.publish(marker);
}

std::vector<double> RobotEnv::tipToGripperTf(const std::vector<double> &tip_tf) {
    tf::Transform tf = utils::listToTf(tip_tf);
    return utils::tfToList(utils::tipToGripperGoal(tf, robo_config_.tip_to_gripper_offset, robo_config_.gripper_to_base_rot_offset));
}

std::vector<double> RobotEnv::gripperToTipTf(const std::vector<double> &tip_tf) {
    tf::Transform tf = utils::listToTf(tip_tf);
    return utils::tfToList(utils::gripperToTipGoal(tf, robo_config_.tip_to_gripper_offset, robo_config_.gripper_to_base_rot_offset));
}
...
geometry_msgs::Twist RobotDiffDrive::calcDesiredBaseTransform(std::vector<double> &base_actions,
    ...
    if (strategy_ == "dirvel") {
        ...
        angle = atan2(vel_forward_vec.x() * goal_vel_vec.y() - vel_forward_vec.y() * goal_vel_vec.x(), vel_forward_vec.dot(goal_vel_vec));
