...
namespace modulation_ellipses {
    ...
    std::vector<ellipse::Ellipse> &Modulation::getEllipses(Eigen::Vector3d &curr_pose, Eigen::VectorXf &curr_speed, Eigen::VectorXd &curr_gripper_pose) {
    ...
    void Modulation::updateSpeedAndPosition(Eigen::Vector3d &curr_pose, Eigen::VectorXf &curr_speed, Eigen::VectorXd &curr_gripper_pose) {
        position_ = curr_pose;
        speed_ = curr_speed;
    ...
    void Modulation::computeGamma() {
        ...
        for (ellipse::Ellipse ellipse : ellipses_) {
            double gamma_i =
                pow(pow((xi_wave_[i][0] / ellipse.getHeight()), 2 * ellipse.getP1()) + pow((xi_wave_[i][1] / ellipse.getWidth()), 2 * ellipse.getP2()), 1.0 / ellipse.getP2());
    ...
    visualization_msgs::MarkerArray Modulation::getEllipsesVisMarker(Eigen::VectorXf &curr_pose, Eigen::VectorXf &curr_speed) {
        ...
        curr_gripper_pose[5] = curr_pose(5);
        curr_gripper_pose[6] = curr_pose(6);
        // std::vector<ellipse_extraction::Ellipse> ellipses = manager_->getEllipses(current_base_pose, curr_speed,curr_gripper_pose,dt);
        // std::vector<ellipse::Ellipse> ellipses = modulation_.getEllipses(current_base_pose, curr_speed,curr_gripper_pose);
        ...
        for (ellipse::Ellipse ellipse : ellipses_) {
            ...
            for (int i = 0; i < max_i; i++) {
                ...
                margin_point << ellipse.getHeight() * cos(theta), ellipse.getWidth() * sign * pow((1.0 - pow(cos(theta), 2.0 * ellipse.getP1())), 1.0 / (2.0 * ellipse.getP2()));
