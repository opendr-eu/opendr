...
GMMPlanner::GMMPlanner(const std::vector<double> &gripper_goal_wrist,
    ...
    BaseGripperPlanner(gripper_goal_wrist,
                       initial_gripper_tf,
                       base_goal,
                       initial_base_tf,
                       success_thres_dist,
    ...
    // afterwards the model can be integrated step by step to generate new gripper poses leading to the correct handling of the object
    tf::Transform gripper_goal_tip = utils::gripperToTipGoal(gripper_goal_wrist_, tip_to_gripper_offset_, gripper_to_base_rot_offset_);
    ...
    prev_plan_.next_gripper_tf = utils::gripperToTipGoal(initial_gripper_tf_, tip_to_gripper_offset_, gripper_to_base_rot_offset_);
...
GripperPlan GMMPlanner::calcNextStep(double time,
                                     double dt,
    ...
    // treat it as a planner that could be pre-computed: calculate next step from transform we wanted to achieve, not the actually achieved
    Eigen::VectorXf current_pose(14);
    current_pose << prev_plan.next_gripper_tf.getOrigin().x(), prev_plan.next_gripper_tf.getOrigin().y(),
        prev_plan.next_gripper_tf.getOrigin().z(), prev_plan.next_gripper_tf.getRotation().x(), prev_plan.next_gripper_tf.getRotation().y(),
        prev_plan.next_gripper_tf.getRotation().z(), prev_plan.next_gripper_tf.getRotation().w(), prev_plan.next_base_tf.getOrigin().x(),
        prev_plan.next_base_tf.getOrigin().y(), 0.0, prev_plan.next_base_tf.getRotation().x(), prev_plan.next_base_tf.getRotation().y(),
...
GripperPlan GMMPlanner::getPrevPlan() {
    ...
    prev_plan_wrist.next_gripper_tf = utils::tipToGripperGoal(prev_plan_wrist.next_gripper_tf, tip_to_gripper_offset_, gripper_to_base_rot_offset_);
    return prev_plan_wrist;
}

GripperPlan GMMPlanner::internalStep(double time,
                                     double dt,
    ...
    GripperPlan next_plan = calcNextStep(time,
                                         dt,
                                         utils::listToVector3(robot_obs.base_velocity),
                                         gripper_velocities.getOrigin(),
                                         gripper_velocities.getRotation(),
                                         prev_plan_,
                                         min_vel,
                                         max_vel,
                                         update_prev_plan);
    ...
    next_plan_wrist.next_gripper_tf = utils::tipToGripperGoal(next_plan.next_gripper_tf, tip_to_gripper_offset_, gripper_to_base_rot_offset_);
