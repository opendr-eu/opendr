...
EEObs BaseGripperPlanner::step(const RobotObs &robot_obs, const double &learned_vel_norm) {
    ...
    if (pause_gripper) {
        ...
        gripper_plan = internalStep(time_planner_ / slow_down_factor_,
...
EEObs BaseGripperPlanner::generateObsStep(const RobotObs &robot_obs){
    GripperPlan gripper_plan = internalStep(time_planner_ / slow_down_factor_,
                                            inStartPause() ? 0.0 : time_step_train_,
                                            robot_obs, -1.0,
                                            false);
