...
LinearPlanner::LinearPlanner(const std::vector<double> &gripper_goal_wrist,
...
tf::Quaternion LinearPlanner::getRot(const tf::Transform &initial,
                                     const tf::Transform &next,
...
GripperPlan LinearPlanner::calcNextStep(const GripperPlan &prev_plan,
                                        const double &dt,
    ...
    // initial_base_tf_.getOrigin()).length()); plan.nextBaseTransform.setRotation(next_base_q);
    return plan;
}

GripperPlan LinearPlanner::internalStep(double time,
                                        double dt,
