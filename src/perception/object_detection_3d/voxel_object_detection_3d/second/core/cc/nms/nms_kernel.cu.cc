...
template<typename DType> __device__ inline DType devIoU(DType const *const a, DType const *const b) {
...
__global__ void nms_kernel(const int n_boxes, const DType nms_overlap_thresh, const DType *dev_boxes,
...
#pragma unroll
    for (int i = 0; i < 5; ++i) {
      block_boxes[threadIdx.x * 5 + i] = dev_boxes[(BLOCK_THREADS * col_start + threadIdx.x) * 5 + i];
    }
  }
  __syncthreads();
...
template<typename DType, int BLOCK_THREADS>
int _nms_gpu(int *keep_out, const DType *boxes_host, int boxes_num, int boxes_dim, DType nms_overlap_thresh, int device_id) {
  ...
  CUDA_CHECK(cudaMemcpy(boxes_dev, boxes_host, boxes_num * boxes_dim * sizeof(DType), cudaMemcpyHostToDevice));
  ...
  nms_kernel<DType, BLOCK_THREADS><<<blocks, threads>>>(boxes_num, nms_overlap_thresh, boxes_dev, mask_dev);
...
// template<>
template int _nms_gpu<float, threadsPerBlock>(int *keep_out, const float *boxes_host, int boxes_num, int boxes_dim,
                                              float nms_overlap_thresh, int device_id);
// template<>
template int _nms_gpu<double, threadsPerBlock>(int *keep_out, const double *boxes_host, int boxes_num, int boxes_dim,
                                               double nms_overlap_thresh, int device_id);