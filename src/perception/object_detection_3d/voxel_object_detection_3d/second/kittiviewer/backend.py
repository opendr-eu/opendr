...
def get_pointcloud():
    ...
    if "annos" in kitti_info:
        ...
        box_np_ops.change_box3d_center_(
            gt_boxes, src=[0.5, 0.5, 0], dst=[0.5, 0.5, 0.5]
        )
        locs = gt_boxes[:, :3]
        dims = gt_boxes[:, 3:6]
        rots = np.concatenate(
            [np.zeros([num_obj, 2], dtype=np.float32), -gt_boxes[:, 6:7]], axis=1
        )
        frontend_annos = {}
    ...
    if "with_det" in instance and instance["with_det"]:
        ...
        dt_boxes_camera = np.concatenate([loc, dims, rots[..., np.newaxis]], axis=1)
        dt_boxes = box_np_ops.box_camera_to_lidar(dt_boxes_camera, rect, Trv2c)
        box_np_ops.change_box3d_center_(
            dt_boxes, src=[0.5, 0.5, 0], dst=[0.5, 0.5, 0.5]
        )
        locs = dt_boxes[:, :3]
        dims = dt_boxes[:, 3:6]
        rots = np.concatenate(
            [np.zeros([num_obj, 2], dtype=np.float32), -dt_boxes[:, 6:7]], axis=1
        )
...
def get_image():
    ...
    if "img_path" in kitti_info:
        ...
        if img_path != "":
            ...
            response["image_b64"] = (
                "data:image/{};base64,".format(image_path.suffix[1:]) +
                response["image_b64"]
            )
            """#
...
def inference_by_idx():
    ...
    t = time.time()
    inputs = BACKEND.inference_ctx.get_inference_input_dict(kitti_info, points)
