...
class WeightedSmoothL1LocalizationLoss(Loss):
  ...
  """
    ...
    """
        ...
        abs_diff_lt_1 = torch.le(abs_diff,
                                 1 / (self._sigma**2)).type_as(abs_diff)
    ...
    """
        ...
        if class_indices is not None:
            weights *= (indices_to_dense_vector(
                class_indices, prediction_tensor.shape[2]).view(
                    1, 1, -1).type_as(prediction_tensor))
    ...
    """
        if self._bootstrap_type == "soft":
            ...
            bootstrap_target_tensor = (
                self._alpha * target_tensor + (1.0 - self._alpha) *
