...
def Augment_train_straight_box_n_kps(object_name, images,scale, bbx_in, grasp_line, group, test):
    ...
    for batch_idx in range(scale):
        ...
        kps = KeypointsOnImage(
            list(Keypoint(x=x_in, y=y_in) for (x_in,y_in) in zip(kps_x_in, kps_y_in)),
            shape=images[0].shape)
        ...
        seq = iaa.Sequential([iaa.Affine(rotate=rt), iaa.Multiply((0.4, 1.5),
                                                    per_channel=0.99),
        iaa.Affine(translate_px={"x": (-30, 30), "y": (-30, 30)}),
                                                    iaa.Affine(scale=(0.75, 1.3))
        ])
