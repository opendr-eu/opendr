...
from projects.simulation.human_model_generation.PIFu.lib.sample_util import save_samples_truncted_prob
from projects.simulation.human_model_generation.PIFu.lib.train_util import reshape_multiview_tensors,\
    reshape_sample_tensor, adjust_learning_rate, calc_error, gen_mesh
from projects.simulation.human_model_generation.PIFu.lib.data import TrainDataset
from projects.simulation.human_model_generation.PIFu.lib.model import HGPIFuNet
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        for train_idx, train_data in enumerate(train_data_loader):
            ...
            if train_idx % opt.freq_plot == 0:
                print(
                    'Name: {0} | Epoch: {1} | {2}/{3} | Err: {4:.06f} | LR: {5:.06f} | Sigma: {6:.02f} | dataT: {'
                    '7:.05f} | netT: {8:.05f} | ETA: {9:02d}:{10:02d}'.format(
