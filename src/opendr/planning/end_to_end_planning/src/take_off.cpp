...
int main(int argc, char **argv)
{
    ...
    // drone pos sub
    ros::Subscriber drone_pos_sub = nh.subscribe<geometry_msgs::PoseStamped>("mavros/local_position/pose", 10, current_pose_callback);
    ...
    while(ros::ok() && !current_state.connected){
      ...
      // we might need to give real pose data here !!!!   /// the very first way point, drone will try to go there !!!!!!! so chose it as target altitude then assign a way point
