...
class BisenetLearner(Learner):
    ...
    def fit(self, dataset, val_dataset, logging_path='', silent=False, verbose=True):
        ...
        self.save(os.path.join(self.temp_path, 'latest'))
    ...
    def infer(self, img):
        ...
        # read csv label path
        label_info = get_label_info(os.path.join(self.temp_path, 'class_dict.csv'))
        # predict
        ...
        spath = os.path.join(self.temp_path, 'heatmap_example.png')
        cv2.imwrite(spath, cv2.cvtColor(heatmap, cv2.COLOR_RGB2BGR))
    ...
    def download(self, path=None, mode="pretrained", verbose=True,
        ...
        if mode == "pretrained":
            ...
            file_path = os.path.join(path, "bisenet_camvid", "bisenet_camvid.pth")
            ...
            file_path = os.path.join(path, "bisenet_camvid", "bisenet_camvid.json")
    ...
    def save(self, path, verbose=True):
        ...
        model_path = os.path.join(path, folder_basename + ".pth")
    ...
    def load(self, path):
        ...
        self.model.module.load_state_dict(torch.load(metadata["model_paths"][0]))
        self.model.eval()
