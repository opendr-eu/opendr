...
int CarmenWrapper::registerLocalizationMessages() {
  ...
  err = IPC_defineMsg(CARMEN_LOCALIZE_GLOBALPOS_NAME, IPC_VARIABLE_LENGTH, CARMEN_LOCALIZE_GLOBALPOS_FMT);
...
// RangeSensor::RangeSensor(std::string name, unsigned int beams_num, unsigned int res, const OrientedPoint& position, double
// span, double maxrange):
...
RangeReading CarmenWrapper::carmen2reading(const carmen_robot_laser_message &msg) {
  ...
  if (msg.laser_pose.theta == msg.robot_pose.theta && !m_frontLaser) {
    ...
    m_frontLaser =
      new RangeSensor(sensorName, msg.num_readings, res, OrientedPoint(0, 0, msg.laser_pose.theta - msg.robot_pose.theta), 0,
...
void CarmenWrapper::publish_particles(carmen_localize_particle_filter_p filter, carmen_localize_summary_p summary) {
  lock();
