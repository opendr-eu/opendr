...
void *GridSlamProcessorThread::fastslamthread(GridSlamProcessorThread *gpt) {
    ...
    gpt->setMatchingParameters(gpt->maxUrange, gpt->maxrange, gpt->sigma, gpt->kernelSize, gpt->lstep, gpt->astep,
                               gpt->iterations, gpt->lsigma, gpt->ogain, gpt->lskip);
...
// 				}
    ...
    gpt->addEvent(done);
    gpt->infoStream()
            << "Hallo, I am the gsp thread. I have finished. Do you think it is the case of checking for unlocked mutexes."
            << endl;
    return 0;
}
...
OrientedPoint
GridSlamProcessorThread::boundingBox(SensorLog *log, double &xmin, double &ymin, double &xmax, double &ymax) const {
    OrientedPoint initialPose(0, 0, 0);
