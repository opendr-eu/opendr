...
void *GridSlamProcessorThread::fastslamthread(GridSlamProcessorThread *gpt) {
  ...
  if (gpt->infoStream()) {
    gpt->infoStream() << " initialPose=" << initialPose.x << " " << initialPose.y << " " << initialPose.theta << cout
...
OrientedPoint GridSlamProcessorThread::boundingBox(SensorLog *log, double &xmin, double &ymin, double &xmax,
                                                   double &ymax) const {
  OrientedPoint initialPose(0, 0, 0);
