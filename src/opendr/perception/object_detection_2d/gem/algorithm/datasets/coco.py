...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        orig_img, target = super(CocoDetection, self).__getitem__(idx)
        image_id = self.ids[idx]
        target = {'image_id': image_id, 'annotations': target}
        # print('image_id: ', image_id)
        img, target = self.prepare(orig_img, target)
        if self._transforms is not None and self.seed is not None:
            img, target = self._transforms(img, target, self.seed)
        else:
            print("Seed not set in dataset_build.")
            return
...
def make_coco_transforms(image_set):
    ...
    if image_set == 'm1_train' or image_set == 'm2_train':
        return T.Compose([
    ...
    if image_set == 'm1_val' or image_set == 'm2_val':
        return T.Compose([
...
def build(img_folder, ann_file, seed, image_set, return_masks):
    dataset = CocoDetection(img_folder, ann_file, seed, transforms=make_coco_transforms(image_set), return_masks=return_masks)
    return dataset
