...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, len_iterator, header=None, verbose=True, silent=False):
        i = 0
        if not header:
            header = ''
        ...
        space_fmt = ':' + str(len(str(len_iterator))) + 'd'
        ...
        MB = 1024.0 * 1024.0
        for m1_obj, m2_obj in iterable:
            data_time.update(time.time() - end)
            yield m1_obj, m2_obj
            iter_time.update(time.time() - end)
            if verbose and (i % print_freq == 0 or i == len_iterator - 1):
                eta_seconds = iter_time.global_avg * (len_iterator - i)
                eta_string = str(datetime.timedelta(seconds=int(eta_seconds)))
                if torch.cuda.is_available():
                    print(log_msg.format(
                        i, len_iterator, eta=eta_string,
                        meters=str(self),
        ...
        if not silent:
            ...
            total_time_msg = '{} Total time: {} ({:.4f} s / it)'.format(
                    header, total_time_str, total_time / len_iterator)
