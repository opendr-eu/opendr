...
namespace at {
  namespace native {
    ...
    std::tuple<Tensor, Tensor> cudnn_grid_sampler_backward(const Tensor &input_t, const Tensor &grid_t,
...
namespace mmdetection {
  ...
  Tensor cudnn_grid_sampler_forward(const Tensor &input_t, const Tensor &grid_t) {
    TensorArg input{contiguousIfZeroInStrides(input_t), "input", 1}, grid{grid_t.contiguous(), "grid", 2};
    ...
    AT_CUDNN_CHECK(cudnnSpatialTfSamplerForward(handle, desc.desc(), &one, idesc.desc(), input->data_ptr(), grid->data_ptr(),
  ...
  std::tuple<Tensor, Tensor> cudnn_grid_sampler_backward(const Tensor &input_t, const Tensor &grid_t,
    ...
    Constant one(dataType, 1);
    Constant zero(dataType, 0);
    AT_CUDNN_CHECK(cudnnSpatialTfSamplerBackward(handle, desc.desc(), &one, idesc.desc(), input->data_ptr(), &zero,
                                                 gdesc.desc(), grad_input_t.data_ptr(), &one, odesc.desc(),
                                                 grad_output->data_ptr(),
                                                 // intruigingly, the outputs don't need descriptors
                                                 grid->data_ptr(), &zero, grad_grid_t.data_ptr()));
