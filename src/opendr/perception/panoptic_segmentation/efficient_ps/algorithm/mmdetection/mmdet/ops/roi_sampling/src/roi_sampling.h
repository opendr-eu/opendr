...
std::tuple<at::Tensor, at::Tensor> roi_sampling_forward_cuda(const at::Tensor &x, const at::Tensor &bbx, const at::Tensor &idx,
                                                             std::tuple<int, int> out_size, Interpolation interpolation,
                                                             PaddingMode padding, bool valid_mask);
...
at::Tensor roi_sampling_backward_cuda(const at::Tensor &dy, const at::Tensor &bbx, const at::Tensor &idx,
                                      std::tuple<int, int, int> in_size, Interpolation interpolation, PaddingMode padding);
...
template<typename scalar_t, typename coord_t, typename index_t, typename Indexer, typename Interpolator> struct Sampler {
  Sampler(Indexer indexer, Interpolator interpolator) : _indexer(indexer), _interpolator(interpolator) {}

  template<typename Accessor> HOST_DEVICE scalar_t forward(coord_t y, coord_t x, Accessor accessor) const {
    ...
    scalar_t p_ll = _indexer.get(accessor, i_l, j_l), p_lh = _indexer.get(accessor, i_l, j_h),
             p_hl = _indexer.get(accessor, i_h, j_l), p_hh = _indexer.get(accessor, i_h, j_h);

    // Step 3: get the interpolated value
    return _interpolator.get(delta_y, delta_x, p_ll, p_lh, p_hl, p_hh);
  }

  template<typename Accessor> HOST_DEVICE void backward(coord_t y, coord_t x, scalar_t grad, Accessor accessor) const {
    // Step 1: find the four indices of the points to read from the input and their offsets
    index_t i_l, i_h, j_l, j_h;
    coord_t delta_y, delta_x;
    _neighbors(y, i_l, i_h, delta_y);
...
template<typename index_t> struct IndexerBase {
...
template<typename scalar_t, typename index_t, PaddingMode padding> struct Indexer;

template<typename scalar_t, typename index_t> struct Indexer<scalar_t, index_t, PaddingMode::Zero> : IndexerBase<index_t> {
  using IndexerBase<index_t>::IndexerBase;

  template<typename Accessor> INLINE_HOST_DEVICE scalar_t get(Accessor accessor, index_t i, index_t j) const {
    return _in_bounds(i, this->_height) && _in_bounds(j, this->_width) ? accessor[i][j] : 0;
  }

  template<typename Accessor> INLINE_HOST_DEVICE void set(Accessor accessor, index_t i, index_t j, scalar_t value) const {
...
private:
  INLINE_HOST_DEVICE bool _in_bounds(index_t k, index_t size) const { return k >= 0 && k < size; }
};

template<typename scalar_t, typename index_t> struct Indexer<scalar_t, index_t, PaddingMode::Border> : IndexerBase<index_t> {
  using IndexerBase<index_t>::IndexerBase;
...
template<typename scalar_t, typename coord_t, Interpolation interpolation> struct Interpolator;

template<typename scalar_t, typename coord_t> struct Interpolator<scalar_t, coord_t, Interpolation::Bilinear> {
  INLINE_HOST_DEVICE scalar_t get(coord_t delta_y, coord_t delta_x, scalar_t p_ll, scalar_t p_lh, scalar_t p_hl,
                                  scalar_t p_hh) const {
  ...
  INLINE_HOST_DEVICE void set(coord_t delta_y, coord_t delta_x, scalar_t value, scalar_t &p_ll, scalar_t &p_lh, scalar_t &p_hl,
                              scalar_t &p_hh) const {
...
template<typename scalar_t, typename coord_t> struct Interpolator<scalar_t, coord_t, Interpolation::Nearest> {
  INLINE_HOST_DEVICE scalar_t get(coord_t delta_y, coord_t delta_x, scalar_t p_ll, scalar_t p_lh, scalar_t p_hl,
                                  scalar_t p_hh) const {
...
template<typename coord_t> INLINE_HOST_DEVICE coord_t img_to_img(coord_t s, coord_t size_in, coord_t size_out) {
  ...
  [&] {                                                                                                                \
    switch (INTERPOLATION) {                                                                                           \
      case Interpolation::Bilinear:                                                                                    \
        TORCH_CHECK(!std::is_integral<scalar_t>::value, "Bilinear interpolation is not available for integral types"); \
        switch (PADDING) {                                                                                             \
