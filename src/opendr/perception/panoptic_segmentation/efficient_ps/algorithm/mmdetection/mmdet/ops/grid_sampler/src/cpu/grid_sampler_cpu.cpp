...
namespace mmdetection {
  ...
  namespace {
    ...
    Tensor grid_sampler_2d_forward_cpu_impl(const Tensor &input, const Tensor &grid,
                                            GridSamplerInterpolation interpolation_mode, GridSamplerPadding padding_mode,
                                            bool align_corners) {
...
#pragma omp parallel for
    ...
    template<typename scalar_t>
    std::tuple<Tensor, Tensor> grid_sampler_2d_backward_cpu_impl(const Tensor &grad_output, const Tensor &input,
                                                                 const Tensor &grid,
...
#pragma omp parallel for
    ...
    template<typename scalar_t>
    std::tuple<Tensor, Tensor> grid_sampler_3d_backward_cpu_impl(const Tensor &grad_output, const Tensor &input,
                                                                 const Tensor &grid,
                                                                 GridSamplerInterpolation interpolation_mode,
                                                                 GridSamplerPadding padding_mode, bool align_corners) {
      auto grad_input = at::zeros_like(input);
...
#pragma omp parallel for
            ...
            for (int64_t w = 0; w < out_W; ++w, gGrid_ptr_NDHW += gGrid_sW /* grad_grid is contiguous */) {
              ...
              if (interpolation_mode == GridSamplerInterpolation::Bilinear) {
                ...
                for (int c = 0; c < C; ++c, gOut_ptr_NCDHW += gOut_sC, gInp_ptr_NC += gInp_sC) {
                  // calculate and set grad_input
                  safe_add_3d(gInp_ptr_NC, iz_nearest, iy_nearest, ix_nearest, gInp_sD, gInp_sH, gInp_sW, inp_D, inp_H, inp_W,
                              *gOut_ptr_NCDHW);
                }
              }
            }
  ...
  Tensor grid_sampler_2d_forward_cpu(const Tensor &input, const Tensor &grid, int64_t interpolation_mode, int64_t padding_mode,
                                     bool align_corners) {
    return AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_2d_forward_cpu", [&] {
      return grid_sampler_2d_forward_cpu_impl<scalar_t>(input, grid, static_cast<GridSamplerInterpolation>(interpolation_mode),
                                                        static_cast<GridSamplerPadding>(padding_mode), align_corners);
    });
  }

  // No shape checking needed here. See # NOTE [ grid_sampler Native Functions ].
  Tensor grid_sampler_3d_forward_cpu(const Tensor &input, const Tensor &grid, int64_t interpolation_mode, int64_t padding_mode,
                                     bool align_corners) {
    return AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_3d_forward_cpu", [&] {
      return grid_sampler_3d_forward_cpu_impl<scalar_t>(input, grid, static_cast<GridSamplerInterpolation>(interpolation_mode),
                                                        static_cast<GridSamplerPadding>(padding_mode), align_corners);
    });
  }

  // No shape checking needed here. See # NOTE [ grid_sampler Native Functions ].
  std::tuple<Tensor, Tensor> grid_sampler_2d_backward_cpu(const Tensor &grad_output, const Tensor &input, const Tensor &grid,
                                                          int64_t interpolation_mode, int64_t padding_mode,
                                                          bool align_corners) {
    return AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_2d_backward_cpu", [&] {
      return grid_sampler_2d_backward_cpu_impl<scalar_t>(grad_output, input, grid,
  ...
  // No shape checking needed here. See # NOTE [ grid_sampler Native Functions ].
  std::tuple<Tensor, Tensor> grid_sampler_3d_backward_cpu(const Tensor &grad_output, const Tensor &input, const Tensor &grid,
                                                          int64_t interpolation_mode, int64_t padding_mode,
                                                          bool align_corners) {
    return AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_3d_backward_cpu", [&] {
      return grid_sampler_3d_backward_cpu_impl<scalar_t>(grad_output, input, grid,
                                                         static_cast<GridSamplerInterpolation>(interpolation_mode),
                                                         static_cast<GridSamplerPadding>(padding_mode), align_corners);
    });
  }
