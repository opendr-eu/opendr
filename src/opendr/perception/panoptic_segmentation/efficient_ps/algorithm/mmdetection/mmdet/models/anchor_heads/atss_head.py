...
from mmdet.core import (PseudoSampler, anchor_inside_flags, bbox2delta, build_assigner, delta2bbox, force_fp32,
...
class ATSSHead(AnchorHead):
    ...
    def loss(self, cls_scores, bbox_preds, centernesses, gt_bboxes, gt_labels, img_metas, cfg, gt_bboxes_ignore=None):
        ...
        cls_reg_targets = self.atss_target(anchor_list,
                                           valid_flag_list,
                                           gt_bboxes,
                                           img_metas,
                                           cfg,
    ...
    def get_bboxes(self, cls_scores, bbox_preds, centernesses, img_metas, cfg, rescale=False):
        ...
        device = cls_scores[0].device
        mlvl_anchors = [
            self.anchor_generators[i].grid_anchors(cls_scores[i].size()[-2:], self.anchor_strides[i], device=device)
        ...
        for img_id in range(len(img_metas)):
            ...
            proposals = self.get_bboxes_single(cls_score_list, bbox_pred_list, centerness_pred_list, mlvl_anchors, img_shape,
    ...
    def atss_target_single(self,
        ...
        if len(pos_inds) > 0:
            pos_bbox_targets = bbox2delta(sampling_result.pos_bboxes, sampling_result.pos_gt_bboxes, self.target_means,
                                          self.target_stds)
