...
class FreeAnchorRetinaHead(RetinaHead):
    def __init__(self,
                 ...
                 alpha=0.5,
                 **kwargs):
        super(FreeAnchorRetinaHead, self).__init__(num_classes, in_channels, stacked_convs, octave_base_scale,
                                                   scales_per_octave, conv_cfg, norm_cfg, **kwargs)
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, cfg, gt_bboxes_ignore=None):
        ...
        for _, (anchors_, gt_labels_, gt_bboxes_, cls_prob_,
                bbox_preds_) in enumerate(zip(anchors, gt_labels, gt_bboxes, cls_prob, bbox_preds)):
            gt_labels_ -= 1

            with torch.no_grad():
                # box_localization: a_{j}^{loc}, shape: [j, 4]
                pred_boxes = delta2bbox(anchors_, bbox_preds_, self.target_means, self.target_stds)
                ...
                if indices.numel() == 0:
                    ...
                    nonzero_box_prob = torch.where((gt_labels_.unsqueeze(dim=-1) == indices[0]), object_box_prob[:, indices[1]],
            ...
            matched_object_targets = bbox2delta(matched_anchors,
                                                gt_bboxes_.unsqueeze(dim=1).expand_as(matched_anchors), self.target_means,
                                                self.target_stds)
