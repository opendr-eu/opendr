...
mask = torch.randn(2, 100, 6, 6, requires_grad=True, device='cuda:0').sigmoid().double()
...
feat = torch.randn(2, 1024, 100, 100, requires_grad=True, device='cuda:0').float()
mask = torch.randn(2, 25, 200, 200, requires_grad=True, device='cuda:0').sigmoid().float()
loop_num = 500
...
for i in range(loop_num):
    ...
    bar.update()
print('\nCARAFE time forward: {} ms/iter | time backward: {} ms/iter'.format((time_forward + 1e-3) * 1e3 / loop_num,
...
for i in range(loop_num):
    ...
    bar.update()
print('\nCARAFE naive time forward: {} ms/iter | time backward: {} ms/iter'.format(
    (time_naive_forward + 1e-3) * 1e3 / loop_num, (time_naive_backward + 1e-3) * 1e3 / loop_num))
